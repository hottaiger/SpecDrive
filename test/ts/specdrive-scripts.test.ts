...
const scriptsDir = path.resolve('assets', 'skills', 'specdrive', 'scripts');

function findUsableBash(): string | null {
  const candidates = [
    process.env.SPECDRIVE_TEST_BASH,
    'bash',
...
async function createChange(tmpDir: string, name: string, yaml: string, tasks = '- [x] done\n') {
  ...
  await writeFile(path.join(changeDir, '.specdrive.yaml'), yaml);
...
describeShell('specdrive shell scripts', () => {
  ...
  beforeEach(async () => {
    ...
    for (const name of [
      'specdrive-env.sh',
      'specdrive-archive.sh',
      'specdrive-guard.sh',
      'specdrive-handoff.sh',
      'specdrive-state.sh',
      'specdrive-yaml-validate.sh',
      'specdrive-hook-guard.sh',
    ]) {
    ...
    guardScript = path.join(tmpScriptsDir, 'specdrive-guard.sh');
    stateScript = path.join(tmpScriptsDir, 'specdrive-state.sh');
    hookGuardScript = path.join(tmpScriptsDir, 'specdrive-hook-guard.sh');
  });
  ...
  it('initializes a new change directory with workflow defaults', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'new-full-change', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes build_pause as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'pause-defaults', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes subagent_dispatch as null for new changes', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'subagent-dispatch-defaults', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as null for full workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-defaults', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes tdd_mode as direct for hotfix workflow', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'tdd-hotfix', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes context_compression as off by default', async () => {
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-defaults', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('snapshots beta context compression from .specdrive/config.yaml when initializing a change', async () => {
    await writeFile(path.join(tmpDir, '.specdrive', 'config.yaml'), 'context_compression: beta\n');

    const result = runBash(tmpDir, stateScript, ['init', 'context-beta', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-beta', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('lets SPECDRIVE_CONTEXT_COMPRESSION override the project context compression default', async () => {
    await writeFile(path.join(tmpDir, '.specdrive', 'config.yaml'), 'context_compression: beta\n');

    const result = runBash(tmpDir, stateScript, ['init', 'context-env', 'full'], {
      SPECDRIVE_CONTEXT_COMPRESSION: 'off',
    });
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'context-env', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('initializes auto_transition from .specdrive/config.yaml when set to false', async () => {
    await fs.mkdir(path.join(tmpDir, '.specdrive'), { recursive: true });
    await writeFile(
      path.join(tmpDir, '.specdrive', 'config.yaml'),
      'context_compression: off\nauto_transition: false\n',
    );

    const result = runBash(tmpDir, stateScript, ['init', 'auto-transition-config-false', 'full']);
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'auto-transition-config-false', '.specdrive.yaml'),
      'utf-8',
    );
    const get = runBash(tmpDir, stateScript, [
      'get',
  ...
  it('next exits non-zero when .specdrive.yaml is missing', async () => {
    const result = runBash(tmpDir, stateScript, ['next', 'next-missing']);

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('.specdrive.yaml not found');
  }, 20_000);

  it('specdrive-env.sh exports bundled script paths from its own directory', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'specdrive-env.sh');
    ...
    await writeFile(
      ...
      [
        ...
        'printf "%s\\n%s\\n%s\\n%s\\n%s\\n" "$SPECDRIVE_STATE" "$SPECDRIVE_GUARD" "$SPECDRIVE_HANDOFF" "$SPECDRIVE_ARCHIVE" "$SPECDRIVE_BASH"',
        '',
      ].join('\n'),
    );
    ...
    expect(result.stdout).toContain('specdrive-state.sh');
    expect(result.stdout).toContain('specdrive-guard.sh');
    expect(result.stdout).toContain('specdrive-handoff.sh');
    expect(result.stdout).toContain('specdrive-archive.sh');
    expect(result.stdout).toContain('bash');
  }, 20_000);

  it('specdrive-env.sh returns failure when a bundled script is missing', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'specdrive-env.sh');
    await fs.rm(path.join(tmpDir, 'scripts', 'specdrive-guard.sh'));
    ...
    expect(result.stderr).toContain('ERROR: SpecDrive scripts not found');
    expect(result.stdout).toContain('source-status=1');
  }, 20_000);

  it('specdrive-env.sh does not change caller shell options when sourced', async () => {
    const envScript = path.join(tmpDir, 'scripts', 'specdrive-env.sh');
  ...
  it('generates a design handoff and requires minimal design doc linkage before leaving design', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/handoff-change/.specdrive/handoff/design-context.json');
  ...
  it('generates a beta spec projection handoff with verbatim spec content', async () => {
    ...
    expect(handoff.status).toBe(0);
    expect(contextPath).toBe('openspec/changes/beta-context/.specdrive/handoff/spec-context.json');
    ...
    expect(contextMarkdown).toContain('Generated-by: specdrive-handoff.sh');
  ...
  it('warns when --full is passed in beta mode', async () => {
    ...
    expect(contextPath).toBe('openspec/changes/beta-full-warn/.specdrive/handoff/spec-context.json');
  }, 20_000);
  ...
  it('rejects invalid build_pause values during schema validation', async () => {
    ...
    expect(result.stderr).toContain('FATAL: .specdrive.yaml schema validation failed');
  }, 20_000);
  ...
  it('keeps optional YAML field reads safe under pipefail', async () => {
    for (const name of ['specdrive-state.sh', 'specdrive-guard.sh']) {
      const content = await fs.readFile(path.join(tmpDir, 'scripts', name), 'utf-8');
  ...
  it('guards bash uname detection when bash cannot be spawned', async () => {
    const files = [
      ...
      path.resolve('test', 'ts', 'specdrive-scripts.test.ts'),
    ];
  ...
  it('uses SPECDRIVE_BASH for nested script calls when PATH bash is unusable', async () => {
    ...
    const result = spawnSync(
      ...
      [
        '-lc',
        [
          'SPECDRIVE_BASH="/bin/bash"',
          `PATH="${toBashPath(fakeBin)}:$PATH"`,
          'export SPECDRIVE_BASH PATH',
    ...
    const yaml = await fs.readFile(
      path.join(tmpDir, 'openspec', 'changes', 'nested-bash', '.specdrive.yaml'),
      'utf-8',
    );
  ...
  it('does not use PATH bash for nested SpecDrive script calls', async () => {
    for (const name of ['specdrive-archive.sh', 'specdrive-guard.sh', 'specdrive-handoff.sh']) {
      const content = await fs.readFile(path.join(tmpDir, 'scripts', name), 'utf-8');

      expect(content, `${name} should use SPECDRIVE_BASH for nested scripts`).not.toMatch(
  ...
  it('reports accurate archive step counts when syncing and annotating', async () => {
    const archiveScript = path.join(tmpDir, 'scripts', 'specdrive-archive.sh');
    ...
    const result = runBash(tmpDir, archiveScript, ['ready-to-archive'], {
      SPECDRIVE_OPENSPEC: toBashPath(fakeOpenSpec),
    });
  ...
  describe('check --recover', () => {
    ...
    it('outputs recovery context for design phase with handoff but no design doc', async () => {
      await createChange(
        ...
        [
          ...
          'plan: null',
          'verify_result: pending',
          'handoff_context: openspec/changes/recover-design/.specdrive/handoff/design-context.json',
    ...
    it('outputs recovery context for archive phase', async () => {
      ...
      expect(result.stdout).toContain('/specdrive-archive');
      expect(result.stdout).toContain('End Recovery Context');
    });
  ...
  describe('review fix: build-complete conditional reset', () => {
    it('preserves verification_report on re-verify cycle (H1)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'reverify-test', '.specdrive.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: verify-fail preserves branch_status', () => {
    it('does not reset branch_status on verify-fail (H6)', async () => {
      ...
      expect(result.status).toBe(0);
      const yaml = await fs.readFile(
        path.join(tmpDir, 'openspec', 'changes', 'branch-preserve', '.specdrive.yaml'),
        'utf-8',
      );
  ...
  describe('review fix: command injection prevention', () => {
    it('rejects build_command with shell metacharacters (C3)', async () => {
      ...
      // No SPECDRIVE_SKIP_BUILD — run_command_string should reject before executing
