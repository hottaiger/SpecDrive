...
describe('run-bats shell runner', () => {
  it('resolves a usable bash instead of directly invoking PATH bash', async () => {
    ...
    expect(content).toContain('process.env.SPECDRIVE_TEST_BASH');
    expect(content).toContain('process.env.SPECDRIVE_BASH');
  ...
  it('rejects WSL bash when resolving bash on Windows', async () => {
    ...
    const shellTests = await fs.readFile(path.resolve('test', 'ts', 'specdrive-scripts.test.ts'), 'utf-8');
  ...
  it('checks explicit SpecDrive bash paths before shelling out to discover fallbacks', async () => {
    const content = await fs.readFile(
      path.resolve('assets', 'skills', 'specdrive', 'scripts', 'specdrive-env.sh'),
      'utf-8',
    );
