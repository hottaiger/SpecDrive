...
感谢你帮助改进 SpecDrive。这份指南偏实操：说明如何配置项目、准备改动、维护分支、提交 PR，以及如何更新 Skill、shell 脚本等 SpecDrive 特有资产。

> 本项目 fork 自 [Comet](https://github.com/rpamis/comet)（原作者 [rpamis](https://github.com/rpamis)）。SpecDrive 在其基础上继续演进。
...
脚本位于 `assets/skills/specdrive/scripts/`，必须兼容 macOS、Linux 和 Windows Git Bash。
...
- 新增脚本必须加入 `test/ts/specdrive-scripts.test.ts` 的 `beforeEach` 拷贝列表。
...
修改 `.specdrive.yaml` 状态文件字段时，需要同步三处：

1. `assets/skills/specdrive/scripts/specdrive-state.sh`：`cmd_set` 白名单与 enum 校验。
2. `assets/skills/specdrive/scripts/specdrive-yaml-validate.sh`：schema 校验与 `KNOWN_KEYS`。
3. `test/ts/specdrive-scripts.test.ts`：测试中的 YAML 示例与断言。
