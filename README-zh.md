...
**SpecDrive 合并了两者的强项**，将核心流程整合为 5 个阶段

主入口 `/comet` 支持当前 Spec 状态检测，适用于长任务——中途关闭当前 AI 编码会话后，回来只需 `/comet`，SpecDrive 会自动读取活跃的
...
长期使用 Skill 的人都知道，这些能力是可以自由组合的，但具体怎么做依然需要真正的实践。SpecDrive 项目可以作为参考：

- **如何稳定触发嵌套 Skill** — 不是让 Agent 依靠文档描述做了“看起来像触发了 Skill”的操作（比如根据 Skill 描述写了文件），而是真正触发
  Skill（核心特征：CC 上有 Skill 触发的打印）。SpecDrive 中会触发大量来自 OpenSpec 和 Superpowers 的能力，这段 Prompt 是怎么写的？

- **如何让组合 Skill 多阶段自动流转** — 不是靠人工介入。SpecDrive 的 5 阶段流程，除必要的用户选择项外，核心流程能够自动进行
  Skill 触发，同时状态机机制也能保障状态扭转的可靠性。

- **如何把 Spec 生命周期做成可恢复流程** — SpecDrive 会把 OpenSpec 的 change/spec 制品与 Superpowers 的设计、计划文档关联起来，并通过
  `.specdrive.yaml` 记录阶段、执行模式、验证结果和归档状态，让 Agent 中断后能够继续，而不是重新翻文档猜进度。

- **如何把文档同步从“用户提醒”变成自动化** — SpecDrive 将 handoff、状态更新、校验和归档同步放进脚本化流程，减少“记得更新 design
  doc”“记得同步 spec”“记得归档 change”这类反复提示。

- **如何设计 Agent 可执行的守护条件** — SpecDrive 的阶段退出不是简单相信 Agent 说“完成了”，而是通过 `specdrive-guard.sh`、
  `specdrive-yaml-validate.sh`、`specdrive-state.sh` 等脚本检查任务、状态字段、验证证据和归档条件，满足条件后才允许推进。

- **如何做跨平台 Skill 分发和安装** — SpecDrive 支持多种 AI 编码平台、项目级/全局安装、中文/英文 Skill 选择，以及平台差异化目录（例如
  Antigravity 的项目级和全局路径不同），可以作为 CLI 安装器和 Skill 打包结构的参考。

- **如何把 shell 脚本写成 Agent 工作流基础设施** — SpecDrive 的脚本需要兼容 macOS、Linux、Windows Git Bash，处理 hash、YAML
...
<details>
<summary><code>specdrive init [path]</code> — 初始化 SpecDrive 工作流</summary>

为选定的 AI 编码平台初始化 OpenSpec、Superpowers 和 SpecDrive 技能。
...
<details>
<summary><code>specdrive status [path]</code> — 显示活跃更改和下一步命令</summary>

显示活跃更改、任务进度，以及推荐的下一步 SpecDrive 工作流命令。
...
<details>
<summary><code>specdrive doctor [path]</code> — 诊断 SpecDrive 安装健康状态</summary>

检查项目级/全局安装、工作目录、已安装技能、脚本和 SpecDrive 状态文件。
...
<details>
<summary><code>specdrive update [path]</code> — 更新 SpecDrive 包和技能</summary>

更新 npm 包，并刷新已检测到的项目级/全局 SpecDrive 技能。
...
|-------------------|------|
| `specdrive --help`    | 显示帮助 |
| `specdrive --version` | 显示版本 |

## 支持平台

`specdrive init` 支持 28 个 AI 编码平台：
...
`specdrive init` 完成后，三组技能将被安装到所选平台的 `skills/` 目录：

### SpecDrive 技能

<details>
<summary>查看 SpecDrive 技能列表</summary>
...
| 脚本                       | 用途                                                                              |
|--------------------------|---------------------------------------------------------------------------------|
| `specdrive-env.sh`           | 脚本发现助手 — 导出 `SPECDRIVE_GUARD`、`SPECDRIVE_STATE`、`SPECDRIVE_HANDOFF`、`SPECDRIVE_ARCHIVE` 等内置脚本路径 |
| `specdrive-guard.sh`         | 阶段转换守护 — 验证退出条件，`--apply` 自动更新 `.specdrive.yaml`                                    |
| `specdrive-handoff.sh`       | 设计交接 — 从 OpenSpec 制品生成带 SHA256 追踪的确定性上下文包                                       |
| `specdrive-archive.sh`       | 一键归档 — 验证状态、同步 specs、移至归档、更新状态                                                  |
| `specdrive-yaml-validate.sh` | 模式校验器 — 校验 `.specdrive.yaml` 结构和字段值                                                 |
| `specdrive-state.sh`         | 统一状态管理 — init/set/get/check/scale，agent 的专属 YAML 接口                             |
| `specdrive-hook-guard.sh`    | 阶段写入守护 — PreToolUse hook，在 open/design/archive 阶段拦截文件写入                         |
...
SpecDrive 使用解耦状态架构，YAML 文件独立管理：

| 文件               | 归属       | 用途              |
|------------------|----------|-----------------|
| `.openspec.yaml` | OpenSpec | Spec 生命周期、变更元数据 |
| `.specdrive.yaml`    | SpecDrive    | 工作流阶段、执行模式、验证状态 |

所有状态和运行阶段都通过脚本更新，并且会在每个阶段退出前校验任务是否真实完成。相比于将复杂状态管理写在 Skill
文本中，脚本化状态机能更稳定地保障阶段流转、YAML 正确性和断点恢复；Agent 只需要通过 SpecDrive 内置命令读取状态，就能知道当前
Spec 处于哪个阶段。

<details>
<summary>查看 .specdrive.yaml 关键字段</summary>
...
`handoff_context` 和 `handoff_hash` 由 `specdrive-handoff.sh` 在离开 design 阶段前写入。项目可在 change 或仓库根配置中设置
...
SpecDrive 通过自动化状态转换确保 agent 执行可靠性：
...
2. **自动化状态转换** — `specdrive-guard.sh --apply` 自动更新 `.specdrive.yaml`
    - 所有阶段转换（open → design/build → verify → archive）使用 `guard --apply`
    - 无需手动状态编辑 — 消除写入验证错误
    - `specdrive-state.sh` 是 agent 对状态操作的专属接口
    - Guard 和 archive 脚本内部使用 `specdrive-state.sh` 进行状态管理

3. **模式校验** — `specdrive-yaml-validate.sh` 确保数据完整性
    - 校验必填字段和可选字段
...
6. **归档自动化** — `specdrive-archive.sh` 一键处理完整归档流程
...
7. **防漂移阶段守护** — 长上下文会话中的阶段意识保障
    ...
    - Hook 层：`specdrive-hook-guard.sh` 在 open/design/archive 阶段硬拦截文件写入（Claude Code 等支持 hook 的平台）
    - 白名单路径：`openspec/*`、`docs/superpowers/*`、`.claude/*`、`.specdrive/*`
...
├── .claude/skills/              # 平台技能目录（SpecDrive + OpenSpec + Superpowers）
│   ├── comet/SKILL.md
│   │   └── scripts/
│   │       ├── specdrive-guard.sh       # 阶段转换守护（--apply 自动更新状态）
│   │       ├── specdrive-env.sh         # 脚本发现助手
│   │       ├── specdrive-handoff.sh     # 设计交接（OpenSpec → Superpowers 上下文追踪）
│   │       ├── specdrive-archive.sh     # 一键归档自动化
│   │       ├── specdrive-yaml-validate.sh # 模式校验器
│   │       ├── specdrive-hook-guard.sh    # 阶段写入守护（PreToolUse hook）
│   │       └── specdrive-state.sh       # 统一状态管理（init/set/get/check/scale）
│   ├── comet-*/SKILL.md
│   ├── openspec-*/SKILL.md
...
│           ├── .specdrive.yaml          # SpecDrive 工作流状态（解耦）
│           ├── proposal.md
│           ├── design.md
...
SpecDrive 支持在 Design → Build 阶段交接时进行上下文压缩。启用后，`specdrive-handoff.sh` 会生成精简的上下文包，在不影响实现正确性的前提下，将
...
启用方式：在 `.specdrive/config.yaml` 中设置 `context_compression: beta`
