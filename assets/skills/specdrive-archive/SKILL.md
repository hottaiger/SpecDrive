...
description: "SpecDrive Phase 5: Archive. Invoke with /specdrive-archive. Merge delta specs into main specs with OpenSpec semantics, archive change."
---

# SpecDrive Phase 5: Archive (Archive)

## Prerequisites

- Verification passed (Phase 4 complete)
- Branch handled
- `verify_result: pass` in `openspec/changes/<name>/.specdrive.yaml`
...
- "Needs adjustment or re-verification" — do not archive; run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> archive-reopen` to return to `phase: verify`, then invoke `/specdrive-verify`. If verification confirms fixes are needed, follow `/specdrive-verify`'s verification-failure decision flow back to `/specdrive-build`
- "Do not archive yet" — do not archive; keep the current `phase: archive` state and wait for the user to invoke `/specdrive-archive` again later

Only after the user selects "Confirm archive" may Step 2 continue. After the user selects "Needs adjustment or re-verification", must first run the `archive-reopen` state transition; do not edit `.specdrive.yaml` manually.
...
6. Update `archived: true` through `specdrive-state transition <archive-name> archived`
