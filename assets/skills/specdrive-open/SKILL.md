---
name: specdrive-open
description: "SpecDrive Phase 1: Open. Invoke with /specdrive-open. Explore ideas through OpenSpec, confirm requirements clarification, then create change structure (proposal + design + tasks)."
---

# SpecDrive Phase 1: Open
...
Full `/specdrive` workflow must not use the Skill tool to load the `openspec-propose` skill by default; only load it when the user explicitly requests generating the proposal and artifacts in one pass.
...
Create `.specdrive.yaml` state file:
...
if [ -z "$SPECDRIVE_STATE" ] || [ -z "$SPECDRIVE_GUARD" ]; then
  echo "ERROR: SpecDrive scripts not found. Ensure the comet skill is installed." >&2
  return 1
fi
...
**Idempotency**: All open phase operations can be safely re-executed. If `.specdrive.yaml` is already at `phase: open` and all three artifact files exist, skip completed steps and continue from the first missing step.
...
- **Phase guard**: Run `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> open --apply`; after all PASS, auto-transitions to next phase

Must use `--apply` before exit, otherwise `.specdrive.yaml` remains at `phase: open` and the next phase entry check will fail.
