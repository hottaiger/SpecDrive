...
description: "SpecDrive Phase 3: Plan and Build. Invoke with /specdrive-build. Create plans and select execution method (subagent or direct) for implementation."
---

# SpecDrive Phase 3: Plan and Build (Build)
...
**Idempotency**: All build phase operations can be safely re-executed. Read `.specdrive.yaml` `phase` field to confirm still in build, read plan header `base-ref`, then use `grep -n '\- \[ \]' tasks.md | head -1` to find the first unchecked task. Already-committed tasks must not be re-committed.
...
| B | Pause to switch model | Record `build_pause: plan-ready`, stop this `/specdrive-build` invocation, and allow the user to resume later from `/specdrive` or `/specdrive-build` |
...
- If the user chooses `executing-plans`: run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> subagent_dispatch null`, then run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode executing-plans`
- If the user chooses `subagent-driven-development`: first confirm the current platform has real background subagent / Task / multi-agent dispatch capability; after confirming, run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> subagent_dispatch confirmed`, then run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> build_mode subagent-driven-development`
...
Run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" set <name> tdd_mode <tdd|direct>`

`isolation` is a script-enforced hard constraint. Full workflow init may temporarily leave it as `null`, but only before this step. If it remains `null`, both the `build → verify` guard and `specdrive-state transition build-complete` will fail.

`subagent_dispatch` is a script-enforced hard constraint. `build_mode: subagent-driven-development` requires `subagent_dispatch: confirmed` before leaving the build phase, otherwise both `specdrive-guard.sh build --apply` and `specdrive-state transition build-complete` will fail.

`tdd_mode` is a script-enforced hard constraint. Full workflow must have `tdd_mode` selected as `tdd` or `direct` before leaving the build phase, otherwise both `specdrive-guard.sh build --apply` and `specdrive-state transition build-complete` will fail.
...
- **branch**: Recommend a branch name based on the workflow type and current date, then let the user confirm or input a custom name. This is a user decision point — **must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm or override the branch name**. Must not skip this step and create the branch directly.
  ...
  - Read the `workflow` field from `.specdrive.yaml` to determine the prefix
...
Requirements:
- the `requesting-code-review` skill must be loaded before `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> build --apply`
...
| Large | Brand-new capability requirements | **Must use the current platform's available user input/confirmation mechanism to pause and wait for the user to explicitly confirm the split**; after user confirms, create independent change through `/specdrive-open` |
...
When creating an independent change, must invoke `/specdrive-open`, not `/opsx:new` directly. `/specdrive-open` creates both OpenSpec artifacts and `.specdrive.yaml`, preventing the new change from leaving the SpecDrive state machine.

**User choices must include**:
- "Split into new change" — create independent change via `/specdrive-open`
...
- **After each task**: immediately check off the corresponding task in the Superpowers plan; if the task maps to OpenSpec tasks.md, also check off the corresponding OpenSpec task; then commit code so `.specdrive.yaml` and file state are durable. Use `grep -c '\- \[ \]' tasks.md` to check remaining unchecked count; no need to re-read the entire file
- **After context compaction**: first run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <change-name> build --recover` — the script outputs structured recovery context (isolation/build_mode status, plan path, task progress, recovery action). Follow the Recovery action to determine next step.
- **User manual-change resume**: handle uncommitted changes through `specdrive/reference/dirty-worktree.md`. That protocol defines checks, attribution, and prohibitions. Build-specific handling:
...
- **Phase guard**: Run `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> build --apply`; after all PASS, state advances to `phase: verify`
...
Configuration can live in the change `.specdrive.yaml`, or in repo-root `.specdrive.yaml` / `comet.yaml` / `.comet.yml` / `comet.yml`.
...
> **Terminology distinction**: the "phase advancement" above is performed by guard `--apply`, which updates the `.specdrive.yaml` `phase` field. This step **always happens** and is not controlled by `auto_transition`. This section's "automatic handoff" only controls whether to automatically invoke the next skill.
