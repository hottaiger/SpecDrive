...
- **阶段退出**: `specdrive-guard <name> <phase> --apply`（必须看到 ALL CHECKS PASSED）
- **压缩恢复**: `specdrive-state check <name> <phase> --recover`
- **状态更新**: 关键操作后通过 `specdrive-state set` 更新字段，禁止手工编辑 .specdrive.yaml
- **handoff 生成**: `specdrive-handoff <name> design --write`（禁止手写摘要）
...
1. 第一个脚本操作 = `specdrive-handoff <name> design --write`（未生成 handoff 禁止加载 brainstorming）
...
1. 第一步运行 `specdrive-state scale <name>` 确定验证级别
...
```bash
"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" check <name> <phase> --recover
```
