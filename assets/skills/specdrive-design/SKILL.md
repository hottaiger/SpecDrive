...
description: "SpecDrive Phase 2: Deep Design. Invoke with /specdrive-design. Produce Design Doc and delta spec through brainstorming."
---

# SpecDrive Phase 2: Deep Design (Design)
...
The script reads the change `.specdrive.yaml` `context_compression` snapshot, then generates and records the matching handoff package.
...
Beta mode (`context_compression: beta` in project `.specdrive/config.yaml`, snapshotted into `.specdrive.yaml` when the change is created) generates:
...
`openspec/changes/<name>/.specdrive/handoff/brainstorm-summary.md` structure:
...
- `openspec/changes/<name>/.specdrive/handoff/design-context.md` (or `spec-context.md` in beta mode)
- `openspec/changes/<name>/.specdrive/handoff/design-context.json` (or `spec-context.json` in beta mode)
...
- `design_doc` written to `.specdrive.yaml`
- **Phase guard**: Run `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> design --apply`; after all PASS, auto-transitions to `phase: build`
