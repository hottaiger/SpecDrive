...
description: "SpecDrive Phase 4: Verify and Close. Invoke with /specdrive-verify. Verify implementation matches design, handle development branch."
---

# SpecDrive Phase 4: Verify and Close (Verify)
...
Before verification begins, handle uncommitted changes through `specdrive/reference/dirty-worktree.md` protocol. Verify phase special handling:
...
- **Fix all**: Run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> verify-fail`, then invoke `/specdrive-build` to fix
...
**When not passing**: Report failures, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/specdrive-build` to fix:
...
When verification does not pass: report missing items, enter Step 1b verification failure decision blocking point. Only after user confirms fix, execute the following command to record failure and roll back to build phase, then invoke `/specdrive-build` to supplement:
...
- If check item 6 finds contradictions (delta spec has content but design doc does not reflect it), **must use the current platform's available user input/confirmation mechanism as a single-select question to pause and wait for the user to choose the handling method**; must not select automatically. Options:
  ...
  - Option B: After user selects B, run `"$SPECDRIVE_BASH" "$SPECDRIVE_STATE" transition <change-name> verify-fail`, then invoke `/specdrive-build`; `/specdrive-build`'s Spec Incremental Update rules will load the Superpowers `brainstorming` skill to update Design Doc + delta spec
...
Verification report must be saved to disk and recorded in `.specdrive.yaml`; after branch handling completes, state fields must also be written. Do not manually set `verify_result: pass`; use guard for auto-transition.
...
- Branch handled
- `verification_report` in `.specdrive.yaml` points to an existing verification report file
- `branch_status: handled` in `.specdrive.yaml`
- **Phase guard**: Run `"$SPECDRIVE_BASH" "$SPECDRIVE_GUARD" <change-name> verify --apply`; after all PASS, auto-transitions to `phase: archive` through `specdrive-state transition verify-pass`
...
Note: after `specdrive-archive` starts, it must first execute the final archive confirmation blocking point and wait for the user to explicitly choose "Confirm archive" before running the archive script. Must not automatically archive just because verification passed.
