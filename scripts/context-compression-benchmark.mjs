...
export async function runBenchmark(options = {}) {
  ...
  const root = path.join(config.workspace, '.specdrive', 'benchmark', 'context-compression');
...
async function generateHandoff(cwd) {
  ...
  if (!bash) {
    throw new Error('Bash or Git Bash is required to generate SpecDrive handoff context');
  }
...
async function runCodexMode({ codexCommand, model, cwd, mode }) {
  const contextFile =
    mode === 'beta'
      ? `openspec/changes/${CHANGE_NAME}/.specdrive/handoff/spec-context.md`
      : `openspec/changes/${CHANGE_NAME}/.specdrive/handoff/design-context.md`;
...
function buildPrompt(mode, contextFile, contextText) {
  return [
    'You are running a local SpecDrive context-compression benchmark.',
    `Mode: ${mode}`,
    `Context source: ${contextFile}`,
    'Use only the inline SpecDrive handoff context below. Do not call tools and do not inspect files.',
