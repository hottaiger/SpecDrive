...
function parseArgs(argv) {
  const options = {
    phase: 'both',
    workspace: path.join(REPO_ROOT, '.specdrive', 'benchmark-runs'),
    repeats: 1,
...
/**
 * L1 fixture: project root with .specdrive config + change directory.
...
async function createL1Fixture(root, mode, tier) {
  ...
  await fs.mkdir(path.join(root, '.specdrive'), { recursive: true });
  await fs.writeFile(path.join(root, '.specdrive', 'config.yaml'), `context_compression: ${mode}\n`);
...
// ── Handoff generation (real specdrive-handoff.sh) ─────────────────────

async function generateHandoff(cwd, changeName = CHANGE_NAME) {
  const bash = findBashCommand();
  if (!bash) throw new Error('Bash or Git Bash is required to generate SpecDrive handoff context');
  const script = path.join(REPO_ROOT, 'assets', 'skills', 'specdrive', 'scripts', 'specdrive-handoff.sh');
...
async function writeSyntheticHandoff(root, mode, tier) {
  ...
  const handoffDir = path.join(root, 'openspec', 'changes', CHANGE_NAME, '.specdrive', 'handoff');
  await fs.mkdir(handoffDir, { recursive: true });
  const contextName = mode === 'beta' ? 'spec-context' : 'design-context';
  await fs.writeFile(
    path.join(handoffDir, `${contextName}.md`),
    [
      mode === 'beta' ? '# SpecDrive Spec Context' : '# SpecDrive Design Handoff',
...
async function measureContext(root, mode) {
  ...
  const content = await fs.readFile(
    path.join(root, 'openspec', 'changes', CHANGE_NAME, '.specdrive', 'handoff', contextName), 'utf-8',
  );
...
async function readHandoffContext(root, mode, changeName = CHANGE_NAME) {
  ...
  const contextPath = path.join(root, 'openspec', 'changes', changeName, '.specdrive', 'handoff', contextFile);
...
function buildDesignPrompt(mode, contextFile, contextText) {
  return [
    'You are a senior engineer running a SpecDrive design phase benchmark.',
    `Mode: ${mode} | Context source: ${contextFile}`,
    '',
    'Read the SpecDrive handoff context below. Then produce a Design Doc that:',
...
function buildBuildPrompt(mode, contextFile, contextText) {
  return [
    'You are implementing a note-board module for a SpecDrive build phase benchmark.',
    `Mode: ${mode} | Context source: ${contextFile}`,
    '',
    'Read the spec acceptance criteria from the SpecDrive handoff context below.',
    '',
    'TASK:',
...
async function createL3Fixture(root, mode, tier) {
  ...
  await fs.writeFile(path.join(changeDir, '.specdrive.yaml'), [
 ...
 * 2. specdrive-handoff.sh compresses context (off=full excerpts, beta=spec-only projection)
...
async function runL3({ claudeCommand, model, cwd, mode, tier, maxRetries }) {
  ...
  // Generate handoff (real specdrive-handoff.sh compression)
...
function renderMarkdownReport(report) {
  const lines = [
    '# SpecDrive Execution Benchmark 报告', '',
...
export async function runExecutionBenchmark(options = {}) {
  const config = {
    ...
    workspace: path.resolve(options.workspace ?? path.join(REPO_ROOT, '.specdrive', 'benchmark-runs')),
  ...
  const root = path.join(config.workspace, '.specdrive', 'benchmark', 'execution');
